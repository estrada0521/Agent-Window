...
fn show_chat_header_menu(
    ...
    .build(&app)
    .map_err(|err| err.to_string())?;
    let log_item = MenuItemBuilder::with_id(
        format!("{}action:revealLog", NATIVE_MENU_PREFIX),
        "Reveal Log",
    )
        ...
        .setup(move |app| {
            ...
            thread::spawn(move || {
                ...
                if repo_root.is_empty() {
                    show_error("Could not find the Agent Window repo.".to_string());
                    return;
                }
                ...
                if hub_ready(hub_port) {
                    ...
                    match cmd.spawn() {
                        ...
                        Err(e) => {
                            ...
                            show_error(format!("Failed to start Hub: {}", e));
                            return;
                        }
                    }
                }
