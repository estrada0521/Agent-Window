...
# Per-event flags signaling that individual paths may have been coalesced or
# dropped, so the watched subtree needs a full rescan instead of trusting the
# reported path list.
KFSEVENTSTREAM_EVENT_FLAG_MUST_SCAN_SUBDIRS = 0x00000001
KFSEVENTSTREAM_EVENT_FLAG_USER_DROPPED = 0x00000002
KFSEVENTSTREAM_EVENT_FLAG_KERNEL_DROPPED = 0x00000004
FSEVENT_RESCAN_FLAGS = (
    KFSEVENTSTREAM_EVENT_FLAG_MUST_SCAN_SUBDIRS
    | KFSEVENTSTREAM_EVENT_FLAG_USER_DROPPED
    | KFSEVENTSTREAM_EVENT_FLAG_KERNEL_DROPPED
)
...
def cf_path_array(cf: ctypes.CDLL, paths: list[str]) -> c_void_p:
    ...
    fn.restype = c_void_p
    cf_strings = [cf_string(cf, path.encode("utf-8")) for path in paths]
    count = len(cf_strings)
    holder = (c_void_p * count)(*cf_strings)
    return fn(None, holder, count, byref(k_cf_type_array_callbacks))
