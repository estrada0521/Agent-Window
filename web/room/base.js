    ...
    const ROOM_BASE_PATH = "__ROOM_BASE_PATH__";
    const ROOM_ASSET_BASE = ROOM_BASE_PATH || "";
    const withRoomBase = (path) => {
      const raw = String(path || "");
      if (!ROOM_BASE_PATH || !raw.startsWith("/") || raw.startsWith("//")) return raw;
      if (raw === ROOM_BASE_PATH || raw.startsWith(`${ROOM_BASE_PATH}/`)) return raw;
      return `${ROOM_BASE_PATH}${raw}`;
    };
    if (ROOM_BASE_PATH) {
      const __origFetch = window.fetch.bind(window);
      window.fetch = (input, init) => {
        if (typeof input === "string" && input.startsWith("/") && !input.startsWith("//")) {
          return __origFetch(withRoomBase(input), init);
        }
        if (input instanceof Request) {
          const url = input.url || "";
          if (url.startsWith(window.location.origin + "/")) {
            const nextUrl = `${window.location.origin}${withRoomBase(url.slice(window.location.origin.length))}`;
...
__INCLUDE:file-icon-theme.js__
    ...
    const fileViewHrefForPath = (path, { embed = false } = {}) => {
      ...
      if (ROOM_BASE_PATH) params.set("base_path", ROOM_BASE_PATH);
      ...
      return withRoomBase(`/file-view?${params.toString()}`);
    };
