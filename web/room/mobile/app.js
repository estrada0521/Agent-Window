...
__INCLUDE:../conversation-state.js__
    ...
    let _hubChildOriW = 0;
    let _hubChildOriH = 0;
    const isEmbeddedHubRoom = window.parent !== window;
    const applyHubIframeLockHeight = () => {
      if (!isEmbeddedHubRoom) {
        syncMainAfterHeight();
        return;
      }
    ...
    const requestHubCloseRoom = () => {
      if (!isEmbeddedHubRoom) return;
      window.parent.postMessage("hub_close_room", "*");
    };
