    const rightMenuBtn = document.getElementById("roomMenuBtn");
    ...
    const openNativeHeaderMenu = async (anchorRect = null) => {
      ...
      window.parent.postMessage({
        type: "show-room-header-menu",
        payload,
      }, "*");
      return true;
    };
    const handleRoomMenuAction = async (payload) => {
      const data = payload || {};
    ...
    window.addEventListener("message", (event) => {
      ...
      void handleRoomMenuAction(event.data.payload);
    });
    window.addEventListener("message", (event) => {
      if (!(event.data && event.data.type === "open-room-header-menu")) return;
    ...
    window.addEventListener("native-menu-action", (event) => {
      void handleRoomMenuAction(event.detail || {});
    });
    ...
    document.getElementById("roomReloadBtn").addEventListener("click", () => void reloadRoom());
    document.getElementById("roomPanelToggle").addEventListener("click", () => toggleDesktopRightPanel());
    ...
    async function runForwardAction(target) {
      ...
      if (action === "openInBrowser") {
        await openExternalLink(`${window.location.protocol}//${window.location.hostname}:__ROOM_PORT__/`).catch(reportExternalLinkFailure);
        return;
      }
