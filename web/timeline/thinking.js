    ...
    const syncThinkingFloatingIcons = () => {
      ...
      if (!root || !messagesEl || !container || !document.body?.classList.contains("agent-running")) {
    ...
    const renderThinkingIndicator = () => {
      ...
      if (!root.querySelector("article.message-row") || !hasRunningRunning) {
        if (existingContainer) {
          ...
          document.dispatchEvent(new CustomEvent("timeline-thinking-updated"));
        }
    ...
    messagesEl?.addEventListener("scroll", scheduleThinkingFloatingIcons, { passive: true });
