    ...
    const fileIconHtml = (rawPath, { isDir = false, classNames = "" } = {}) => {
      ...
      const url = withTimelineBase(def.iconPath || `/file-icon-theme/icon/${encodeURIComponent(iconId)}`);
    ...
    const ensureFileIconTheme = () => {
      ...
      fileIconThemeState.loadPromise = fetch(withTimelineBase("/file-icon-theme"))
        .then((res) => {
