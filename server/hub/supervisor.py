...
from server.timeline.control import (
    SessionControlError,
    create_session,
    kill_session,
...
def timeline_server_state_matches(hub, state: dict | None, *, workspace: str) -> bool:
    if not state:
        return False
...
def timeline_launch_env(hub) -> dict[str, str]:
    env = os.environ.copy()
...
def ensure_timeline_server(
    ...
    *,
    ...
    with lock:
        ...
        same_server = timeline_server_state_matches(hub, state, workspace=resolved_workspace)
        if same_server and bool(state.get("active")) == bool(expected_active):
            return True, timeline_port, ""
        if same_server:
            stop_ok, stop_detail = stop_timeline_server(resolved_workspace)
            if not stop_ok:
                return False, timeline_port, stop_detail
        elif not port_is_bindable(timeline_port):
            return False, timeline_port, f"timeline port {timeline_port} is occupied"

        if not expected_active:
            stop_detail = stop_inactive_timeline_servers(keep_workspace=resolved_workspace)
            if stop_detail:
                return False, timeline_port, stop_detail

        env = timeline_launch_env(hub)
        try:
            process = launch_timeline_server(resolved_workspace, env=env)
        except OSError as exc:
            return False, timeline_port, str(exc)

        def _ready() -> bool:
            state = read_timeline_server_state(timeline_port)
            return (
                bool(state)
                and timeline_server_state_matches(hub, state, workspace=resolved_workspace)
                and bool(state.get("active")) == bool(expected_active)
            )

        detail = wait_for_timeline_server(process, _ready)
        return not detail, timeline_port, detail


def revive_timeline(hub, timeline_name: str) -> tuple[bool, str]:
    ...
    timeline_port = workspace_timeline_port(workspace)
    if not port_is_bindable(timeline_port):
        return False, f"timeline port {timeline_port} is occupied"
    try:
        create_session(
...
def archive_timeline(hub, timeline_name: str) -> tuple[bool, str]:
    live = live_timelines_query(hub)
    if live.state == "unhealthy":
        raise TmuxUnhealthy(live.detail)
    if timeline_name not in live.workspaces:
        return False, "That active timeline is not available in this repo."
    try:
        kill_session(timeline_name=timeline_name)
    except SessionControlError as exc:
        return False, str(exc)
    return True, ""


def delete_archived_timeline(hub, timeline_name: str) -> tuple[bool, str]:
    live = live_timelines_query(hub)
    if live.state == "unhealthy":
        raise TmuxUnhealthy(live.detail)
    if timeline_name in live.workspaces:
