...
from server.timeline.routes.assets import dispatch_get_assets_route
from server.timeline.routes.read import dispatch_get_read_route
from server.timeline.routes.write import dispatch_post_write_route
from server.timeline.assets import TimelineAssets
...
def _not_initialized(*_args, **_kwargs):
    raise RuntimeError("timeline_server.initialize_from_argv() must run before serving requests")
...
def initialize_from_argv(argv: list[str] | None = None) -> None:
    ...
    if len(argv) != 1:
        raise SystemExit("usage: python -m server.timeline.server <workspace>")
...
timeline_restart_pending = False
timeline_restart_lock = threading.Lock()
timeline_restart_release_event = threading.Event()
server = None


def queue_timeline_restart():
    global timeline_restart_pending
    with timeline_restart_lock:
        if timeline_restart_pending:
            return False, "restart already pending", False
        timeline_restart_pending = True

    env = _restart_env()
    server.shutdown()
    server.server_close()
    try:
        process = launch_timeline_server(workspace, env=env)
    except OSError as exc:
        return False, str(exc), True
    expected_workspace = str(Path(workspace).expanduser().resolve())

    def _ready() -> bool:
        reported = read_timeline_server_state(port)
...
def release_timeline_restart() -> None:
    timeline_restart_release_event.set()


def _route_context() -> dict:
    current_timeline_name, _current_log_path = state.timeline_binding_snapshot()
    return {
        ...
        "files": files,
        "assets": assets,
        "render_timeline_html_fn": render_timeline_html,
        "queue_timeline_restart_fn": queue_timeline_restart,
        "release_timeline_restart_fn": release_timeline_restart,
    }
...
def main(argv: list[str] | None = None) -> None:
    ...
    if timeline_restart_pending:
        timeline_restart_release_event.wait()
