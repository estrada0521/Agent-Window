...
from server.timeline.file_view_scripts import (
...
_FONT_FACES_CSS = (_REPO_ROOT / "web" / "timeline" / "font-faces.css").read_text()
INLINE_PROGRESSIVE_PREVIEW_MAX_BYTES = 512 * 1024
PROGRESSIVE_TEXT_PREVIEW_CHUNK_BYTES = 32 * 1024


def _timeline_markdown_preview_css() -> str:
    theme_vars_css = (_REPO_ROOT / "web/timeline/markdown-theme-vars.css").read_text(encoding="utf-8")
...
def _timeline_markdown_frontmatter_js() -> str:
    return (_REPO_ROOT / "web/timeline/markdown-frontmatter.js").read_text(encoding="utf-8")


def _timeline_file_link_parse_js() -> str:
...
def render_file_view(
    ...
    *,
    ...
    font_base = prefix or ""
    font_face_css = apply_font_tokens(_FONT_FACES_CSS.replace("__TIMELINE_BASE_PATH__", font_base))
...
const TIMELINE_BASE_PATH = __previewBasePath || "";
{file_link_parse_js}
