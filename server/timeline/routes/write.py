...
def _post_reload_timeline(handler, _parsed, ctx) -> None:
    owns_restart = False
    try:
        ok, detail, owns_restart = ctx["queue_timeline_restart_fn"]()
        handler._send_json(
            200 if ok else 503,
        ...
        if owns_restart:
            ctx["release_timeline_restart_fn"]()
...
def _post_add_agent(handler, _parsed, ctx) -> None:
    ...
    state = ctx["state"]
    try:
        instance = add_agent(timeline_name=ctx["timeline_name"], agent=agent)
...
def _post_remove_agent(handler, _parsed, ctx) -> None:
    ...
    state = ctx["state"]
    try:
        instance = remove_agent(timeline_name=ctx["timeline_name"], agent=agent)
    except Exception as exc:
        handler._send_json(500, {"ok": False, "error": str(exc)})
        return
...
_POST_ROUTES = {
    "/reload-timeline": _post_reload_timeline,
